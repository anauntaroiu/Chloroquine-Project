...
## Load Data
Drug_list_1 <- as.matrix(read.csv("CQ 24h, replicate 1.csv"))
Drug_list_2 <- as.matrix(read.csv("CQ 24h, replicate 2.csv"))
Drug_list_3 <- as.matrix(read.csv("CQ 24h, replicate 3.csv"))
Drug_list_4 <- as.matrix(read.csv("CQ 24h, replicate 4.csv"))
Drug_list_5 <- as.matrix(read.csv("CQ 24h, replicate 5.csv"))
Drug_list_6 <- as.matrix(read.csv("CQ 24h, replicate 6.csv"))

###########################################################
# Create function to replace missing expression values with 
# lowest recorded values across replicates

## INPUTS: Six replicates from one sample

## OUTPUTS: Replicate with replaced missing values; index positions
## where no data was measured for all replicates

replace_missing_values <- function(Rep1, Rep2, Rep3, Rep4, Rep5, Rep6){
  ...
  for (i in 1:length(Rep1)){ # Loop through gene IDs in replicate 1
    ...
    if (is.na(Rep1[i])){ # If expression data is missing
      ...
      if (is.na(Rep2[i]) && is.na(Rep3[i]) && is.na(Rep4[i]) && is.na(Rep5[i]) && is.na(Rep6[i])){ # If exprs data is missing for all Reps
        ...
        recorded_values <- c() # Object to hold replicates with recorded values
        
        if (!is.na(Rep2[i])){ # If replicate 2 is recorded 
          recorded_values <- cbind(recorded_values, Rep2[i]) # Save it
        }
        
        if (!is.na(Rep3[i])){ # If replicate 3 is recorded 
          recorded_values <- cbind(recorded_values, Rep3[i]) # Save it
        }
        
        if (!is.na(Rep4[i])){ # If replicate 4 is recorded 
          recorded_values <- cbind(recorded_values, Rep4[i]) # Save it
        }
        
        if (!is.na(Rep5[i])){ # If replicate 5 is recorded 
          recorded_values <- cbind(recorded_values, Rep5[i]) # Save it
        }
        
        if (!is.na(Rep6[i])){ # If replicate 6 is recorded 
          recorded_values <- cbind(recorded_values, Rep6[i]) # Save it
        }
        
        Rep1[i] <- min(recorded_values) # Save lowest recorded value
    }
   }
  }
...
Drug_Replicate_1 <- replace_missing_values(Drug_list_1, Drug_list_2, Drug_list_3,
                                           Drug_list_4, Drug_list_5, Drug_list_6)
Drug_Replicate_2 <- replace_missing_values(Drug_list_2, Drug_list_1, Drug_list_3,
                                           Drug_list_4, Drug_list_5, Drug_list_6)
Drug_Replicate_3 <- replace_missing_values(Drug_list_3, Drug_list_1, Drug_list_2,
                                           Drug_list_4, Drug_list_5, Drug_list_6)
Drug_Replicate_4 <- replace_missing_values(Drug_list_4, Drug_list_1, Drug_list_3,
                                           Drug_list_2, Drug_list_5, Drug_list_6)
Drug_Replicate_5 <- replace_missing_values(Drug_list_5, Drug_list_1, Drug_list_3,
                                           Drug_list_4, Drug_list_2, Drug_list_6)
Drug_Replicate_6 <- replace_missing_values(Drug_list_6, Drug_list_1, Drug_list_3,
                                           Drug_list_4, Drug_list_5, Drug_list_2)
...
replace_missing_values_No_Drug <- function(Rep1, Rep2, Rep3){
...
No_Drug_Replicate_1 <- replace_missing_values_No_Drug(No_Drug_list_1, No_Drug_list_2, No_Drug_list_3)
No_Drug_Replicate_2 <- replace_missing_values_No_Drug(No_Drug_list_2, No_Drug_list_1, No_Drug_list_3)  
No_Drug_Replicate_3 <- replace_missing_values_No_Drug(No_Drug_list_3, No_Drug_list_2, No_Drug_list_1)

###########################################################
## Remove missing data probes from Drug/No Drug Data

cut <- c(Drug_Replicate_1[[2]], No_Drug_Replicate_1[[2]], 
         Drug_Replicate_2[[2]], No_Drug_Replicate_2[[2]],
         Drug_Replicate_3[[2]], No_Drug_Replicate_3[[2]],
         Drug_Replicate_4[[2]], Drug_Replicate_5[[2]],
         Drug_Replicate_6[[2]])
...
Drug_Replicate_4 <- Drug_Replicate_4[[1]][-c(cut),]
Drug_Replicate_5 <- Drug_Replicate_5[[1]][-c(cut),]
Drug_Replicate_6 <- Drug_Replicate_6[[1]][-c(cut),]

##############################################################
# Load Gene IDs for each probe
Probe_names <- as.matrix(read.csv("Probe Information.csv"))
Probe_names <- Probe_names[-c(cut),]
...
for (i in 1:dim(Probe_names)[1]){ # Loop through each probe ID
  ...
  if (length(hits) > 1){ # If a repeat was found
    ...
    for (k in 1:length(hits)){ # Loop through repeated probes
      ...
      x <- c(Drug_Replicate_1[k], Drug_Replicate_2[k], Drug_Replicate_3[k], Drug_Replicate_4[k], Drug_Replicate_5[k], Drug_Replicate_6[k])
    ...
    Drug_Replicate_4[hits[1]] <- Drug_Replicate_4[hits[k]]
    Drug_Replicate_5[hits[1]] <- Drug_Replicate_5[hits[k]]
    Drug_Replicate_6[hits[1]] <- Drug_Replicate_6[hits[k]]
    ...
    Drug_Replicate_4 <- Drug_Replicate_4[-c(hits[2:length(hits)])]
    Drug_Replicate_5 <- Drug_Replicate_5[-c(hits[2:length(hits)])]
    Drug_Replicate_6 <- Drug_Replicate_6[-c(hits[2:length(hits)])]
    ...
    # Place the probe values for the probe with the highest variance in the first probe in the lists
...
Exprs_matrix <- cbind(2^Drug_Replicate_1, 2^Drug_Replicate_2, 2^Drug_Replicate_3, 
                      2^Drug_Replicate_4, 2^Drug_Replicate_5, 2^Drug_Replicate_6,
                      2^No_Drug_Replicate_1, 2^No_Drug_Replicate_2, 2^No_Drug_Replicate_3)
rownames(Exprs_matrix) <- Probe_names[,2]
colnames(Exprs_matrix) <- c("Drug_rep1","Drug_rep2", "Drug_rep3", "Drug_rep4","Drug_rep5", "Drug_rep6","No_Drug_rep1", "No_Drug_rep2", "No_Drug_rep3")

# Remove probes with missing gene IDs
...
design <- model.matrix(~0+factor(c("Drug", "Drug", "Drug", "Drug",
                                   "Drug", "Drug", "Control", "Control", "Control")))
...
write.csv(MADE_Data, file = "Expression_Data_For_MADE_CQ_24hr.csv")
...
write.csv(nonsignificant_genes, file = "Nonsignificant_Genes_For_CQ_24hr.csv")

write.csv(Diff_Exprs_Genes, file = "Diff_Exp_Genes_For_CQ_24hr.csv")
